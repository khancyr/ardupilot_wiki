...
def create_vehicle_git_repos():
    ...
    def _make_repo(vehicle: str):
        ...
        try:
            ...
            return vehicle, temp_repo_dir

        except Exception as e:
            error(f"Failed to create temp repo for {vehicle}: {e}")
            # Fallback to using the main repo
            return vehicle, BASEPATH

    # perform creations in parallel threads; each invocation is independent
    with ThreadPoolExecutor(max_workers=len(VEHICLES)) as executor:
        futures = {executor.submit(_make_repo, v): v for v in VEHICLES}
        for fut in futures:
            veh, repo = fut.result()
            vehicle_repos[veh] = repo
